...
const sub = wskit.subscribe('chats.$chatId', {
  type: 'presence', params: { chatId: '7onDwfuO6MkXNtLXby1Gg' }
});
```
...
// @include: sub
// ---cut---
sub.whisper('userIsTyping', { userId: '019a3266-c935-7a3c-8659-fc3167baf68d' });
```
...
<Tabs items={['Using returned unsubscribe function', 'Using original callback reference']}>
  ...
  <Tabs.Tab>
    ...
    // ---cut---
    /** You have to explicitly define the type of the event **/
    const userIsTypingHandler = (event: { userId: string }) => {
      /** Do something with the event **/
    }
